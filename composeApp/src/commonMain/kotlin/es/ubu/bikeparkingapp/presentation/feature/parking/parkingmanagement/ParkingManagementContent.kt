...
fun ParkingManagementContent(
    ...
    if (state.showDeactivateDialog) {
        AlertDialog(
            onDismissRequest = actions.onDeactivateDialogDismiss,
            title = { Text(stringResource(Res.string.deactivate_parking)) },
            text = { Text(stringResource(Res.string.deactivate_parking_confirm)) },
            confirmButton = {
                TextButton(onClick = actions.onDeactivateConfirm) {
                    Text(
            ...
            dismissButton = {
                TextButton(onClick = actions.onDeactivateDialogDismiss) {
    ...
    if (state.showToggleDialog) {
        AlertDialog(
            onDismissRequest = actions.onToggleServiceDismiss,
            title = { Text(stringResource(Res.string.toggle_state)) },
            text = { Text(stringResource(Res.string.toggle_state_confirm)) },
            confirmButton = {
                TextButton(onClick = actions.onToggleServiceConfirm) {
                    Text(
                        stringResource(Res.string.toggle_state),
                        color = MaterialTheme.colorScheme.error
                    )
                }
            },
            dismissButton = {
                TextButton(onClick = actions.onToggleServiceDismiss) {
                    Text(stringResource(Res.string.cancel))
                }
            }
        )
            ...
            Column(
                ...
                Row(
                    ...
                    if (parking?.isOperative == true) {
                        OutlinedButton(
                            onClick = actions.onViewOccupancyClick,
