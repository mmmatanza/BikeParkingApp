    ...
    fun loadParkingArea(parkingAreaId: String) {
        viewModelScope.launch {
            ...
            getParkingAreaByIdUseCase(parkingAreaId)
                ...
                .onFailure { error ->
                    when (error){
                        is NoNetworkException -> _state.value = _state.value.copy(error = error, isLoading = false)
                        else -> _state.value = _state.value.copy(error = Exception(error.message), isLoading = false)
                    }
                }
        }
    }
